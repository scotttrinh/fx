...
test "terminal tool schema exposes a compatible object and exact internal action branches" {
    ...
    try std.testing.expectEqualSlices([]const u8, &terminal_model_required, model_input.required);
    ...
    try std.testing.expectEqual(terminal_actions.len, model_action.enum_values.len);
    try std.testing.expectEqualSlices(
        []const u8,
        &.{ "native", "tmux" },
        schemaProperty(model_input, "backend").?.enum_values,
    );
    try std.testing.expectEqualStrings(
        "Required for wait; required for start when return_when is non-immediate; maximum blocking time in milliseconds.",
        schemaProperty(model_input, "wait_ceiling_ms").?.description,
    );
    try std.testing.expectEqualStrings(
        "Required for session-targeted actions. Set null for start and list; owner-catalog authority is private.",
        schemaProperty(model_input, "session_id").?.description,
    );
